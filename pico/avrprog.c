...
// Read 3 signature bytes from the device into `signature[0..2]`.
// Uses the standard ISP "Read Signature" command (0x30, 0x00, addr, 0x00).
...
uint16_t avr_read_program_memory(uint16_t word_address) {
    uint16_t data; // 16 bit value: high<<8 | low
...
void avr_write_temporary_buffer_page(uint16_t* data, size_t data_len) {
    ...
    // Allow callers to supply device-appropriate page sizes. The caller
    // must ensure the supplied length does not exceed the device page word count.
